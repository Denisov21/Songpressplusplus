...
```
✅  Pacchetto creato: build_deb/songpressplusplus_8.0.0_all.deb
```
...
│   └── songpressplusplus-7.0.0-py3-none-any.whl
└── songpressplusplus_7.0.0_all.deb   ← pacchetto finale da installare
```
...
> **⚠️ Nota:** il numero di versione (`8.0.0`) è solo un **esempio** ed è **da verificare**: usa quello effettivamente prodotto dallo script, mostrato a schermo al termine della build.

```bash
sudo dpkg -i "build_deb/songpressplusplus_8.0.0_all.deb"
```
...
>
> ```bash
> sudo apt install ./songpressplusplus_8.0.0_all.deb
> ```
>
> Il prefisso `./` (o un percorso completo) è **obbligatorio**: senza almeno una `/` nel nome, `apt` interpreta l'argomento come il nome di un pacchetto da cercare nei repository e restituisce "impossibile trovare il pacchetto". Se non sei nella cartella del `.deb`, passa il percorso completo, ad esempio `sudo apt install ~/…/build_deb/songpressplusplus_8.0.0_all.deb`.
...
```toml
[project]
version = "8.0.0"   # ← modifica questo numero
```
