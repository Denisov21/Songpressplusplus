...
| `ScreenRuler.py` | Mixin `ScreenRulerMixin` che aggiunge il comando **Strumenti › Righello a schermo** (F9, voce XRC `screenRuler`; se assente dall'XRC viene creata da codice): righello a schermo stile PowerToys per misurare larghezza e altezza di testo ed elementi visibili. Cattura lo schermo e lo mostra a tutto schermo con una barra di strumenti: modalità *Riquadro* (con "Adatta al contenuto"), *Spaziatura*, *Orizzontale*, *Verticale*; tolleranza colore, unità (px, pt, mm, cm, in), croce di misura attivabile, zoom della vista 1×–16× che non altera le misure, copia della misura negli appunti. Su Wayland la cattura usa grim/spectacle, xdg-desktop-portal (via PyGObject o `gdbus`) o gnome-screenshot. Icone: `img/screenRuler.png`, `img/close.png`. Ereditato da `SongpressFrame` ed attivato tramite `self._InstallScreenRuler()`. Eseguibile anche da solo per test: `python3 ScreenRuler.py`. |
...
| `ScreenRuler.po` / `.mo` | `ScreenRuler.py` — righello a schermo: voce di menu, modalità di misura, opzioni della barra (adatta al contenuto, croce, tolleranza, unità, zoom), tooltip e messaggio di errore di cattura. |
