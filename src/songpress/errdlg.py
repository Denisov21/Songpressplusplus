...
# Modified by Luca Allulli (just minor adaptations) and Denisov21             #
...
def _read_pyproject():
    """Legge nome e versione da pyproject.toml risalendo dalla directory del file."""
    try:
        import tomllib
    except ImportError:
        try:
            ...
            return None, None
    # pyproject.toml si trova 3 livelli sopra: src/songpress/errdlg.py -> pyproject.toml
    ...
    for _ in range(4):
        ...
        if os.path.isfile(candidate):
            try:
                ...
                return project.get('name', None), project.get('version', None)
            except Exception:
                return None, None
...
_PROG_NAME    = _PROG_NAME    or u"Songpress++"
_PROG_VERSION = _PROG_VERSION or u""
...
class ErrorDialog(wx.Dialog):
    ...
    def GetProgramName(self):
        ...
        """
        if _PROG_VERSION:
            return u"%s Version: %s" % (_PROG_NAME, _PROG_VERSION)
        return _PROG_NAME
