...
class SongpressFrame(SDIMainFrame):
    ...
    def OnRestart(self, evt):
        """Chiede conferma e riavvia l'applicazione tramite os.execl()."""
        d = wx.MessageDialog(
            self.frame,
    ...
    def OnGuideMarkdown(self, evt):
        ...
        # wx.Locale.GetCanonicalName restituisce es. "it_IT", "en_US", "fr_FR"
        locale = wx.GetLocale()
        lang_code = ''
        if locale:
        ...
        dlg = wx.Dialog(self.frame, title=_("Quick guide - Songpress++"),
                        size=(720, 580), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        ...
        btn = wx.Button(dlg, wx.ID_OK, _("Close"))
        sizer.Add(btn, 0, wx.ALIGN_RIGHT | wx.ALL, 6)
        dlg.SetSizer(sizer)
        dlg.Layout()
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def _md_to_html(self, md_text):
        import re
        lines = md_text.split('\n')
        html_lines = ['<html><body style="font-family:Arial,sans-serif;margin:16px;font-size:13px;">']
        in_code = False
        in_table = False
        for line in lines:
            if line.startswith('```'):
            ...
            if line.startswith('### '):
                ...
                l = re.sub(r'`(.+?)`', r'<code style="background:#f0f0f0;padding:1px 3px;">\1</code>', l)
