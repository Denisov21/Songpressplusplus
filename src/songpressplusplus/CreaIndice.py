...
def _make_watermark(path, opacity, angle):
    ...
    the resulting picture behind the text. Raises RuntimeError if Pillow is
    missing.
    """
    try:
...
class IndexPanel(wx.Panel):
    ...
    def _build_ui(self, default_folder):
        ...
        self.extButtons[0][0].SetValue(True)  # .cho preselected
        ...
        orow.Add(wx.StaticText(sb, label=_(u"Opacity (%):")),
                 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT | wx.RIGHT, 6)
        self.opacitySpin = wx.SpinCtrl(sb, min=0, max=100, initial=15,
                                       size=(_SPIN_W, -1))
        orow.Add(self.opacitySpin, 0, wx.RIGHT, 12)
        orow.Add(wx.StaticText(sb, label=_(u"Angle (\u00b0):")),
                 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 6)
        self.angleSpin = wx.SpinCtrl(sb, min=-180, max=180, initial=45,
                                     size=(_SPIN_W, -1))
        orow.Add(self.angleSpin, 0, wx.RIGHT, 12)
        orow.Add(wx.StaticText(sb, label=_(u"Size (%):")),
                 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 6)
        self.sizeSpin = wx.SpinCtrl(sb, min=1, max=100, initial=60,
                                    size=(_SPIN_W, -1))
...
def showCreateIndexDialog(parent, default_folder=None):
